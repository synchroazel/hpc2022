...
int save_tuning_table(std::string save_tune_dir_path, double *final_tuning_table, int tuning_table_rows,
                      int tuning_table_columns, int linear_rows, int radial_rows, int sigmoid_rows,
                      int polynomial_rows) {
...
void tune_radial(Dataset *df_train,
    ...
    for (int i = start; i < end; i++) {
        ...
        double coef0 = 0;
        double degree = 0;
        double params[4] = {combination_matrix[index(i - start, 0, 2)], combination_matrix[index(i - start, 1, 2)],
                            coef0, degree};
...
void tune_polynomial(Dataset *df_train,
    ...
    if ((current_process - process_offset) >
        (cost_array_size * gamma_array_size * coef0_array_size * degree_array_size)) {
        return;
    }
    ...
    for (int i = 0; i < cost_array_size; i++) {
        for (int j = 0; j < gamma_array_size; j++) {
            for (int k = 0; k < coef0_array_size; k++) {
                ...
                std::cout << "Process " << current_process << " training (sigmoid) with cost " << params[0] << " , gamma " << params[1] << " and intercept " << params[2] << std::endl;
#endif
                parallel_train(*df_train, &svm, params, lr, limit, process_offset, available_processes, false, "", 0,
                               eps);
        ...
        for (int j = 0; j < gamma_array_size; j++) {
            for (int k = 0; k < coef0_array_size; k++) {
                for (int l = 0; l < degree_array_size; l++) {
                    ...
                    std::cout << "Process " << current_process << " training (polynomial) with cost " << params[0] << " , gamma " << params[1] << ", intercept " << params[2]  << " and degree " << params[3] << std::endl;
#endif
                    parallel_train(*df_train, &svm, params, lr, limit, process_offset, available_processes, false, "",
                                   0, eps);

                    parallel_test(*df_validation, &svm, process_offset, available_processes);

                    if (current_process == process_offset) {
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 0,
                                           result_table_columns)] = params[0];
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 1,
                                           result_table_columns)] = params[1];
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 2,
                                           result_table_columns)] = params[2];
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 3,
                                           result_table_columns)] = params[3];

                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 4,
                                           result_table_columns)] = svm.accuracy;
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 5,
                                           result_table_columns)] = svm.accuracy_c1;
                        result_table[index(offset + i * gamma_array_size * coef0_array_size * degree_array_size +
                                           j * coef0_array_size * degree_array_size + k * degree_array_size + l, 6,
                                           result_table_columns)] = svm.accuracy_c2;
                    }
                    free(svm.arr_alpha_s);
                    free(svm.arr_alpha_s_in);
